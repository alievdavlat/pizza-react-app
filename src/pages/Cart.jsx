...
function Cart() {
  ...
  const {items, totolPrice} = useSelector(state => state.cart)
  const totolCount = items.reduce((sum , item ) => sum + item.count, 0)

const onclickClear = () => {
  if(window.confirm('are you sure to clear all pizzas?')) dispatch(clearItems())
       ...
       <div className="content__items">
        ...
        <div className="cart__bottom">
          <div className="cart__bottom-details">
            <span> Всего пицц: <b>{totolCount} шт.</b> </span>
            <span> Сумма заказа: <b>{totolPrice} ₽</b> </span>
          </div>
