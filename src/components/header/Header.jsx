...
function Header() {
  const {totolPrice, items} = useSelector((state) => state.cart)
  const { inputValue, setInputValue , loading, setLoading}  = React.useContext(context)

  const totolCount = items.reduce((sum , item ) => sum + item.count, 0)
  return (
    <div className="header">
    <div className="container">
{ 
  loading ?  <Skeleton width={250} height={60}/>  : 
  <Link to={'/'}>
...
{
loading ? <Skeleton width={300} height={60}/> :  <SearchInput inputValue={inputValue} setInputValue={setInputValue}/>
}
<div className="header__cart">
{ loading ? <Skeleton width={170} height={50} style={{borderRadius:'20px'}}/>  : 
  <Link to={'/cart'}>
